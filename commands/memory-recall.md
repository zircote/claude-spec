...
Examples:
- `/recall "database selection criteria"`
- `/recall "authentication" --spec=user-auth --type=decision`
- `/recall "why did we" --full --limit=5`
</command_arguments>
...
To see full content: /recall "${QUERY}" --full
To explore a specific memory: /recall --id=${MEMORY_ID}
```
...
```
Next actions:
  [1] View full content of result #N: /recall --id=<id> --full
  [2] Refine search with filters: /recall "${QUERY}" --spec=<slug>
  [3] Load all context for a spec: /context <spec-slug>
```
...
3. Check if memories exist: /memory status
4. Rebuild index if needed: /memory reindex
