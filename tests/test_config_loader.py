...
class TestGetUserConfigPath:
    ...
    def test_returns_path_in_home(self):
        ...
        assert path == Path.home() / ".claude" / "claude-spec.config.json"


class TestGetTemplateConfigPath:
    """Tests for get_template_config_path."""

    def test_returns_path_to_default_config(self):
        """Test returns path to default config at plugin root."""
        path = get_template_config_path()
        assert path.name == "claude-spec.config.json"
...
class TestGetCommandSteps:
    ...
    def test_returns_steps_for_command(self, tmp_path, monkeypatch):
        ...
        result = get_command_steps("claude-spec:complete", "preSteps")
...
class TestGetEnabledSteps:
    ...
    def test_filters_disabled_steps(self, tmp_path, monkeypatch):
        ...
        result = get_enabled_steps("claude-spec:complete", "preSteps")
        assert len(result) == 2
...
class TestGetEnabledStepsUnknownPhase:
    ...
    def test_returns_empty_for_unknown_phase(self, tmp_path, monkeypatch):
        ...
        result = get_enabled_steps("claude-spec:complete", "unknownPhase")
        assert result == []
    ...
    def test_returns_empty_for_invalid_phase_type(self, tmp_path, monkeypatch):
        ...
        assert get_enabled_steps("claude-spec:complete", "during") == []
        assert get_enabled_steps("claude-spec:complete", "beforeSteps") == []
        assert get_enabled_steps("claude-spec:complete", "afterSteps") == []
    ...
    def test_handles_empty_steps_list(self, tmp_path, monkeypatch):
        ...
        assert get_enabled_steps("claude-spec:complete", "preSteps") == []
        assert get_enabled_steps("claude-spec:complete", "postSteps") == []
...
class TestGetCommandStepsEdgeCases:
    ...
    def test_returns_empty_for_missing_phase(self, tmp_path, monkeypatch):
        ...
        result = get_command_steps("claude-spec:complete", "postSteps")
        assert result == []
    ...
    def test_returns_steps_for_all_valid_phases(self, tmp_path, monkeypatch):
        ...
        pre_result = get_command_steps("claude-spec:complete", "preSteps")
        post_result = get_command_steps("claude-spec:complete", "postSteps")
