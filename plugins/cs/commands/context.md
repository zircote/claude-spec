...
Examples:
- `/cs:context user-auth`
- `/cs:context api-design --recent=5`
</command_arguments>
...
1. ${UNRESOLVED_COUNT} blocker(s) need resolution
   → /cs:recall "blocker" --spec=${SPEC_SLUG} --full

2. Continue from last progress point:
   → Task ${LAST_TASK_ID}: ${LAST_TASK_SUMMARY}

3. Review key decisions before proceeding:
   → /cs:recall "decision" --spec=${SPEC_SLUG}

Ready to continue implementation? Run /cs:i ${SPEC_SLUG}
```
...
|-------|---------|----------|
| Spec not found | "No spec found matching '${SLUG}'" | "List available: /cs:s --list" |
| No memories | "No memories captured for ${SLUG}" | "Memories capture as you work" |
| Index empty | "Memory index is empty" | "Run /cs:memory reindex" |
</error_handling>
