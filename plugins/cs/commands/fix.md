...
Running in QUICK MODE with defaults:
   ...
   * Learning Capture: Enabled
   ...
   Use `/cs:fix` without --quick for interactive mode.
```
</quick_mode>
  ...
  questions=[
    {
      ...
      "multiSelect": false,
      "options": [
        {"label": "CODE_REVIEW.md (Recommended)", "description": "Use findings from recent /cr or /cs:review"},
...
- Run `/cs:fix --from-memory` later to address them
${END IF}
...
- Addressing findings captured by `/cs:review` over time
